...
% Default is short. To build long from the repo root with Make:
%   make -C documents long
% Direct manual compile example:
%   lualatex -jobname=ShawalMbalireCV_long "\def\cvmode{long} \input{documents/ShawalMbalireCV.tex}"
% Data-driven sections are populated from JSON under documents/assets/data/* with Lua helpers.
...
\usepackage{fontspec}
% Choose clean sans + mono (fallbacks if unavailable will revert to Latin Modern)
...
\ifthenelse{\equal{\cvmode}{short}}{
  % Short mode (trimmed experience list, no projects / publications / education)
  ...
  % Long mode (full experience list, projects, education, skills, publications)
  \directlua{ generate_personal_common('assets/data/personal.json') }
